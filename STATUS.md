...
### 1. **בדיקה אמיתית על אתרים** ✅ נבדק (2026-07-01)
> נבדק עם Playwright + `--load-extension` על YouTube (חיפוש) ו-X.com בפועל: קליפי מופיע, מסביר, שואל, ומזהה קטגוריה נכון (ראה `verify-realsite.js`).
> באג אמיתי שנמצא ותוקן: מסכת ה-SVG לא תאמה את דמות קליפי מהאתר — הוחלפה לעיצוב העניבה המטאלית הזהה.
> באג נוסף שתוקן: `bundle-entry.js` קרא ל-`dismissPost()` בלי לחכות לתוצאה ותמיד הצהיר "הסרתי בשמך" גם כשההסרה נכשלה — עכשיו ממתין ל-`result.ok` ואומר משפט כן לפי מה שבאמת קרה.

### 2. **action-engine לפלטפורמות החדשות** ⚠️ חלקית נבדק
> **באג אמיתי נמצא ותוקן:** Meta העביר את Threads מ-`threads.net` ל-`threads.com` — ה-manifest, site-adapters ו-action-engine עדיין הצביעו רק על הדומיין הישן, כך שהתוסף לא פעל בכלל עבור משתמשי Threads אמיתיים. תוקן ב-2026-07-01 (שני הדומיינים נתמכים כעת).
> Reddit חוסם דפדפנים אוטומטיים (Bot detection) — לא ניתן לבדוק אוטומטית בלי לעקוף חסימה, מה שלא נעשה בכוונה.
> LinkedIn/Threads דורשים חשבון מחובר לבדיקת זרימת ה"הסתרה" המלאה — לא ניתן לבדוק בלי להזין פרטי התחברות אמיתיים. הקוד עבר בדיקת עין נוספת אך לא בדיקת DOM חי.
> DOM של הפלטפורמות משתנה תכופות, ייתכן שיצטרכו עדכון תקופתי.
