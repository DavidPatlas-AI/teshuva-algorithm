...
- **LinkedIn verified against the live DOM 2026-07-28 (logged-in account, real feed) — was actually broken, now fixed.** LinkedIn rewrote its frontend with hashed CSS-in-JS class names (`_2fb87c58`-style); `feed-shared-update-v2`/`occludable-update`/`[data-urn]`/`feed-shared-control-menu__trigger` no longer exist anywhere in the post DOM. `container()` was returning null on every post, so `dismissPost()` never even reached `menuBtn()`. The stable signal is `role="listitem"` on the post's ancestor. Also, the real control-menu button's aria-label is dynamic Hebrew text (`"פתיחת תפריט הבקרה לפוסט מאת [author]"`), not the hardcoded `"פתח תפריט בקרה"` the old code checked for exactly, and the English-only regex fallback never matched it either — added a `תפריט הבקרה` substring alternative. `option()` was already correct as-is: menu items do carry `role="menuitem"`, and the real "Not interested" text is exactly `לא מעוניין`. Re-verify if LinkedIn ships another redesign — their class names are clearly generated/unstable across releases, `role="listitem"` is the only thing worth trusting long-term.
- Reddit blocks automated browsers with a "Prove your humanity" JS challenge (confirmed 2026-07-28) — can't be verified via Playwright or Claude-in-Chrome either, this isn't fixable from our side.
- Threads: verified 2026-07-28 that the menu opens and the button/menu selectors work, but **only while logged out** could "Not interested"/"Hide"/"Mute"/"Block" not be reached — an anonymous session's post menu only offers "Copy link". Needs re-verification with a logged-in account to confirm the actual moderation options render with the expected text.
